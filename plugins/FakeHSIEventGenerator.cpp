...
void
FakeHSIEventGenerator::init(const dunedaq::dal::DaqModule* conf)
{
  m_conf = conf->cast<dunedaq::dal::FakeHSIEventGeneratorModule>();
  TLOG_DEBUG(TLVL_ENTER_EXIT_METHODS) << get_name() << ": Entering init() method";
  // THIS CALL WILL NEED AN OKS VERSION 
  // (or else we put a hack here to replace it, but inelegant and same call 
  // is used in HSIReadout)
...
FakeHSIEventGenerator::do_configure(const nlohmann::json& obj)
{
  ...
  // Otherwise use json for configuration
  else {
